...
class computeErrorField(object):
    ...
    def __init__(self, obsf, adcf, meta, yamlname=os.path.join('/home/jtilson/ADCIRCSupportTools', 'config', 'err.yml'),
        ...
        utilities.log.debug('input OBS wl data are {}'.format(str(self.df_adc_wl)))
    ...
    def _intersectionTimes(self):
        ...
        #print('Number of intersected times is '+str(len(inttimes)))
...
# Many edge cases can mess this procedure up
    ...
    def _outputDataToFiles(self, metadata='_test'):
        ...
        self.finalfilename=utilities.writeCsv(self.df_final,rootdir=self.rootdir,subdir='errorfield',fileroot='stationSummaryAves',iometadata=metadata)
        self.cyclefilename=utilities.writeCsv(self.df_cycle_avgs,rootdir=self.rootdir,subdir='errorfield',fileroot='stationPeriodAves',iometadata=metadata)
        self.metafilename=utilities.writeCsv(df_metadata,rootdir=self.rootdir,subdir='errorfield',fileroot='stationMetaData',iometadata=metadata)
        self.mergedname=utilities.writeCsv(self.df_merged,rootdir=self.rootdir,subdir='errorfield',fileroot='adc_obs_error_merged',iometadata=metadata)
        self.errorfilename=utilities.writePickle(self.diff,rootdir=self.rootdir,subdir='errorfield',fileroot='tideTimeErrors',iometadata=metadata)
    ...
    def executePipeline(self, metadata = 'Nometadata' ):
        ...
        dummy = self._outputDataToFiles(metadata=metadata)
...
def main(args):
    ...
    rootdir=utilities.fetchBasedir(config['DEFAULT']['RDIR'],basedirExtra='TestComputeError')
    #
    ...
    dummy = cmp._outputDataToFiles(metadata='_maintest') # Note the delimiter is added here
