...
# get station-to-node indices for grid
    ...
    if adc.config["ADCIRC"]["fortNumber"] == "63":
        if args.ignore_pkl or not os.path.exists(ADCfile):
            ...
            ##df = get_water_levels63(adc.urls, node_idx, station_ids)
            df = adc.get_water_levels63(node_idx, station_ids)
            df = df.sort_index()
...
class Adcirc:
    ...
    def get_water_levels63(self, nodes, stationids):
        """
        ...
        for datecyc, url in urls.items():
            ...
            if url is None:
                ...
                if "zeta" not in nc.variables.keys():
                    ...
                    print('{}'.format(type(df_sub.index[0])))
                    ##tin = datetime.strptime(df_sub.index[0],'%Y-%m-%d %H:%M:%S')
                    ##tout = datetime.strptime(df_sub.index[-1],'%Y-%m-%d %H:%M:%s')
                    print("df_sub time range = {} -> {}" .format(df_sub.index[0], df_sub.index[-1]))
                    if initialtime==None:
                        initialtime = df_sub.index[0]
                        finaltime = df_sub.index[-1]
                    else:
                        initialtime = df_sub.index[0] if (df_sub.index[0] < initialtime) else initialtime
                        finaltime = df_sub.index[-1] if (df_sub.index[-1] > finaltime) else finaltime
                    df = pd.concat((df, df_sub), axis=0)
                    #utilities.log.debug(" df_cat time range = {} -> {}" .format(df.index[0], df.index[-1]))
                    utilities.log.debug(" df_concat final time range = {} -> {}" .format(df.index[0], df.index[-1]))
    # update T1 and T2 to reflect current data sets
        utilities.log.info('Updating internal time range to reflect data values')
        self.T1=df.index[0]
        self.T2=df.index[-1]
        print('water_levels_63: final times {} {}'.format(df.index[0], df.index[-1]))
        return df

    def get_water_levels61(self, stationids):
        """
