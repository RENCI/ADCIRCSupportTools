...
def build_slurm(ROOTDIR,YAMLNAME,ERRFILES,CLAMPFILE,ADCJSON,RANGE,SILL):
    slurm = list()
    slurm.append('#!/bin/sh')
    ...
    slurm.append('ERRFILE=$(sed -n "$SLURM_ARRAY_TASK_ID"p '+ERRFILES) # SLURM_ARRAY_TASK_ID Not defined yet phug
    ##Grab a value out of the list usinmg the ID (0 or 12 as a start : ITS was different than RENCI)
...
def main(args):
    ...
    numfs = len(listedFiles.items())
    print('Number of files found {}'.format(numfs))

    import random
    #for key, value in dict(random.sample(listedFiles.items(),20)).items(): # listedFiles.items():
    
    ERRFILES=listedFiles.items()
    print('type list {}'.format(type(ERRFILES)))
    print('listy is '.format(ERRFILES))
    utilities.log.info('ERRFILES {}'.format(ERRFILES))
    utilities.log.info('CLAMPFILE {}'.format(CLAMPFILE))
    utilities.log.info('ADCJSON {}'.format(ADCJSON))
    utilities.log.info('YAMLNAME {}'.format(YAMLNAME))
    utilities.log.info('ROOTDIR {}'.format(ROOTDIR))
    utilities.log.info('RANGE {}'.format(RANGE))
    utilities.log.info('SILL {}'.format(SILL))
    slurmFilename = build_slurm(ROOTDIR,YAMLNAME,ERRFILES,CLAMPFILE,ADCJSON,RANGE,SILL)
    cmd = 'sbatch --array=0-'+numfs+'%10 ./'+slurmFilename
    os.system(cmd)
