...
def build_slurm(ROOTDIR,YAMLNAME,ERRFILE,CLAMPFILE,ADCJSON,RANGE):
    slurm = list()
    slurm.append('#!/bin/sh')
    ...
    slurm.append('python -u $dir/krigListOfErrorSets.py  --daily --inrange "'+RANGE+'" --outroot "'+ROOTDIR+'" --yamlname "'+YAMLNAME+'" --errorfile "'+ERRFILE+'" --clampfile "'+CLAMPFILE+'" --gridjsonfile "'+ADCJSON+'"' )
...
def main(args):
    ...
    for key, value in listedFiles.items():
        ...
        utilities.log.info('ROOTDIR {}'.format(ROOTDIR))
        utilities.log.info('RANGE {}'.format(RANGE))
        slurmFilename = build_slurm(ROOTDIR,YAMLNAME,ERRFILE,CLAMPFILE,ADCJSON,RANGE)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inrange', action='store', dest='inrange',default=2, help='If specified then an internal config is constructed', type=int)
